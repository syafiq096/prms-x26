...
import { PassengerEntity, ResourceCategory, ResourceEntity, SystemState, AuditEventEntity, AuditActorType } from '../database/entities';
...
import { HistorySort, Interaction, InteractionOutcome, MembershipUsage, ReportingFilter as ReportingFilterValue, ReportingService, ReportingWindow as ReportingWindowValue, UsageSummary } from '../application/reporting/reporting.service';
...
enum ActorRole { CREW_LEAD = 'CREW_LEAD', PASSENGER = 'PASSENGER' }
...
@ObjectType() class CurrentActor { @Field(() => ID) id!: string; @Field(() => ActorRole) role!: ActorRole; @Field() displayName!: string; @Field() active!: boolean; }
@ObjectType() class ResourceUsage { @Field(() => ID) id!: string; @Field(() => ID) passengerId!: string; @Field(() => ID) resourceId!: string; @Field() passengerMissionCode!: string; @Field(() => MembershipLevel) passengerMembershipLevel!: MembershipLevel; @Field() resourceCode!: string; @Field() resourceDisplayName!: string; @Field(() => ResourceCategory) resourceCategory!: ResourceCategory; @Field(() => MembershipLevel) resourceMinimumMembershipLevel!: MembershipLevel; @Field(() => ResourceStatus) resourceStatus!: string; @Field() occurredAt!: Date; }
@ObjectType() class UseResourcePayload { @Field() allowed!: boolean; @Field(() => ResourceUsage, { nullable: true }) usage!: ResourceUsage | null; @Field(() => String, { nullable: true }) denialReason!: string | null; }
@ObjectType() class AuditEvent { @Field(() => ID) id!: string; @Field() eventType!: string; @Field() result!: string; @Field(() => String, { nullable: true }) reasonCode!: string | null; @Field(() => AuditActorType) actorType!: AuditActorType; @Field(() => ID, { nullable: true }) crewLeadActorId!: string | null; @Field(() => ID, { nullable: true }) passengerActorId!: string | null; @Field(() => ID, { nullable: true }) crewLeadSubjectId!: string | null; @Field(() => ID, { nullable: true }) passengerSubjectId!: string | null; @Field(() => ID, { nullable: true }) resourceSubjectId!: string | null; @Field(() => ID, { nullable: true }) resourceUsageSubjectId!: string | null; @Field(() => Date) occurredAt!: Date; }
@ObjectType() class AuditEventEdge { @Field() cursor!: string; @Field(() => AuditEvent) node!: AuditEvent; }
@ObjectType() class AuditEventConnection { @Field(() => [AuditEventEdge]) edges!: AuditEventEdge[]; @Field(() => PageInfo) pageInfo!: PageInfo; @Field(() => Int) totalCount!: number; }
@ObjectType() class ReportingWindow { @Field() from!: Date; @Field() to!: Date; }
@ObjectType() class ResourceInteraction { @Field(() => ID) id!: string; @Field(() => InteractionOutcome) outcome!: InteractionOutcome; @Field(() => String, { nullable: true }) denialReason!: string | null; @Field() occurredAt!: Date; @Field(() => String, { nullable: true }) passengerMissionCode!: string | null; @Field(() => MembershipLevel, { nullable: true }) passengerMembershipLevel!: MembershipLevel | null; @Field(() => ID, { nullable: true }) resourceId!: string | null; @Field(() => String, { nullable: true }) resourceCode!: string | null; @Field(() => String, { nullable: true }) resourceDisplayName!: string | null; @Field(() => ResourceCategory, { nullable: true }) resourceCategory!: ResourceCategory | null; @Field(() => MembershipLevel, { nullable: true }) resourceMinimumMembershipLevel!: MembershipLevel | null; @Field(() => ResourceStatus, { nullable: true }) resourceStatus!: string | null; }
@ObjectType() class ResourceInteractionEdge { @Field() cursor!: string; @Field(() => ResourceInteraction) node!: ResourceInteraction; }
@ObjectType() class ResourceInteractionConnection { @Field(() => [ResourceInteractionEdge]) edges!: ResourceInteractionEdge[]; @Field(() => PageInfo) pageInfo!: PageInfo; @Field(() => Int) totalCount!: number; }
@ObjectType() class UsageReportSummary { @Field(() => ReportingWindow) window!: ReportingWindow; @Field(() => Int) allowedCount!: number; @Field(() => Int) deniedCount!: number; @Field(() => Int) totalAttempts!: number; @Field() denialRate!: number; }
@ObjectType() class MembershipUsageGroup { @Field(() => MembershipLevel) membershipLevel!: MembershipLevel; @Field(() => Int) allowedCount!: number; @Field(() => Int) deniedCount!: number; @Field(() => Int) totalAttempts!: number; }
@ObjectType() class MembershipUsageReport { @Field(() => ReportingWindow) window!: ReportingWindow; @Field(() => [MembershipUsageGroup]) groups!: MembershipUsage[]; }
@ObjectType() class ResourceDemandRow { @Field(() => ID) resourceId!: string; @Field() resourceCode!: string; @Field() resourceDisplayName!: string; @Field(() => ResourceCategory) resourceCategory!: ResourceCategory; @Field(() => MembershipLevel) resourceMinimumMembershipLevel!: MembershipLevel; @Field(() => Int) allowedCount!: number; @Field(() => Int) deniedCount!: number; @Field(() => Int) totalAttempts!: number; }
@ObjectType() class ResourceDemandEdge { @Field() cursor!: string; @Field(() => ResourceDemandRow) node!: ResourceDemandRow; }
@ObjectType() class ResourceDemandConnection { @Field(() => [ResourceDemandEdge]) edges!: ResourceDemandEdge[]; @Field(() => PageInfo) pageInfo!: PageInfo; @Field(() => Int) totalCount!: number; }
@ObjectType() class ResourceDemandReport { @Field(() => ReportingWindow) window!: ReportingWindow; @Field(() => ResourceDemandConnection) demand!: ResourceDemandConnection; }
...
@InputType() class UseResourceInput { @Field(() => ID) @IsUUID() resourceId!: string; @Field() @IsUUID() idempotencyKey!: string; }
@InputType() class ReportingWindowInput { @Field() @IsDate() from!: Date; @Field() @IsDate() to!: Date; }
@InputType() class ReportingFilter { @Field(() => [InteractionOutcome], { nullable: true }) @IsOptional() @IsEnum(InteractionOutcome, { each: true }) outcomes?: InteractionOutcome[] | null; @Field(() => [MembershipLevel], { nullable: true }) @IsOptional() @IsEnum(MembershipLevel, { each: true }) membershipLevels?: MembershipLevel[] | null; @Field(() => [ResourceCategory], { nullable: true }) @IsOptional() @IsEnum(ResourceCategory, { each: true }) categories?: ResourceCategory[] | null; @Field(() => [String], { nullable: true }) @IsOptional() @IsString({ each: true }) denialReasons?: string[] | null; @Field(() => String, { nullable: true }) @IsOptional() @IsString() resourceText?: string | null; }
...
export class PrmsResolver {
  constructor(private readonly systemStatusQuery: SystemStatusQueryService, private readonly crewLeadQuery: CrewLeadQueryService, private readonly passengerQuery: PassengerQueryService, private readonly resourceQuery: ResourceQueryService, private readonly discovery: ResourceDiscoveryService, private readonly setup: SystemSetupService, private readonly crewLeads: CrewLeadsService, private readonly passengersService: PassengersService, private readonly resourcesService: ResourcesService, private readonly usage: ResourceUsageService, private readonly auditEventQueries: AuditEventQueryService, private readonly reporting: ReportingService, private readonly actors: ActorContextService) {}
  ...
  @Query(() => CurrentActor) async currentActor(@Context() context: RequestContext): Promise<CurrentActor> { const actor = await this.actors.authenticated(context); if (actor.type === 'CREW_LEAD') { const lead = await this.crewLeadQuery.byId(actor.id); return { id: lead.id, role: ActorRole.CREW_LEAD, displayName: lead.fullName, active: lead.active }; } const passenger = await this.passengerQuery.byId(actor.id); return { id: passenger.id, role: ActorRole.PASSENGER, displayName: passenger.fullName, active: passenger.active }; }
  ...
  @Query(() => AuditEventConnection) async auditEvents(@Args('page', { type: () => PageInput, nullable: true }) page: PageInput | undefined, @Context() context: RequestContext): Promise<AuditEventConnection> { await this.actors.crewLead(context); return auditEventConnection(await this.auditEventQueries.list(page ?? {})); }
  @Query(() => ResourceInteractionConnection) async myUsageHistory(@Args('window') window: ReportingWindowInput, @Args('filter', { type: () => ReportingFilter, nullable: true }) filter: ReportingFilter | undefined, @Args('sort', { type: () => HistorySort, nullable: true, defaultValue: HistorySort.NEWEST }) sort: HistorySort, @Args('page', { type: () => PageInput, nullable: true }) page: PageInput | undefined, @Context() context: RequestContext): Promise<ResourceInteractionConnection> { const passenger = await this.actors.passenger(context); return interactionConnection(await this.reporting.history(passenger.id, window as ReportingWindowValue, filter as ReportingFilterValue ?? {}, sort, page ?? {})); }
  @Query(() => UsageReportSummary) async usageReportSummary(@Args('window') window: ReportingWindowInput, @Args('filter', { type: () => ReportingFilter, nullable: true }) filter: ReportingFilter | undefined, @Context() context: RequestContext): Promise<UsageReportSummary> { await this.actors.crewLead(context); return this.reporting.summary(window as ReportingWindowValue, filter as ReportingFilterValue ?? {}) as Promise<UsageSummary>; }
  @Query(() => MembershipUsageReport) async usageByMembership(@Args('window') window: ReportingWindowInput, @Args('filter', { type: () => ReportingFilter, nullable: true }) filter: ReportingFilter | undefined, @Context() context: RequestContext): Promise<MembershipUsageReport> { await this.actors.crewLead(context); return this.reporting.membership(window as ReportingWindowValue, filter as ReportingFilterValue ?? {}); }
  @Query(() => ResourceDemandReport) async resourceDemand(@Args('window') window: ReportingWindowInput, @Args('filter', { type: () => ReportingFilter, nullable: true }) filter: ReportingFilter | undefined, @Args('page', { type: () => PageInput, nullable: true }) page: PageInput | undefined, @Context() context: RequestContext): Promise<ResourceDemandReport> { await this.actors.crewLead(context); const result = await this.reporting.demand(window as ReportingWindowValue, filter as ReportingFilterValue ?? {}, page ?? {}); return { window: result.window, demand: { edges: result.connection.edges, pageInfo: { hasNextPage: result.connection.hasNextPage, endCursor: result.connection.endCursor }, totalCount: result.connection.totalCount } }; }
  ...
  @Mutation(() => UseResourcePayload) async useResource(@Args('input') input: UseResourceInput, @Context() context: RequestContext): Promise<UseResourcePayload> { const result = await this.usage.record(await this.actors.passenger(context), input.resourceId, input.idempotencyKey); return result.allowed ? { allowed: true, usage: result.usage, denialReason: null } : { allowed: false, usage: null, denialReason: result.reason }; }
}
...
function auditEventConnection(value: { edges: { node: AuditEventEntity; cursor: string }[]; hasNextPage: boolean; endCursor: string | null; totalCount: number }): AuditEventConnection { return { edges: value.edges, pageInfo: { hasNextPage: value.hasNextPage, endCursor: value.endCursor }, totalCount: value.totalCount }; }
function interactionConnection(value: { edges: { node: Interaction; cursor: string }[]; hasNextPage: boolean; endCursor: string | null; totalCount: number }): ResourceInteractionConnection { return { edges: value.edges, pageInfo: { hasNextPage: value.hasNextPage, endCursor: value.endCursor }, totalCount: value.totalCount }; }
